...
Slime.prototype.actualizarEnergía = function(incremento){
    this.energía = incremento > 0 ? Math.min(this.energía + incremento, MAX_ENERGIA) : Math.max(this.energía + incremento, 0);
}

// Ejercicio 3
Slime.prototype.atacar = function(objetivo){
    if(this.energía > 0){
        let dañoRealizado = daño(this, objetivo);
        let energiaRobada = Math.min(objetivo.energía, dañoRealizado);
        objetivo.energía -= energiaRobada;
        this.energía = Math.min(this.energía + energiaRobada, MAX_ENERGIA);
    }
}

// Ejercicio 4
Slime.prototype.duelo = function(oponente){
    let energiaInicial = oponente.energía;
    this.atacar(oponente);
    oponente.atacar(this);
    if(energiaInicial > oponente.energía){
        this.nivel++;
        return this;
    } else {
        oponente.nivel++;
...
SlimeSanador.prototype = new Slime();

SlimeSanador.prototype.curar = function(paciente){
    paciente.energía = Math.min(paciente.energía + this.poder*this.nivel, MAX_ENERGIA);
}

// Ejercicio 6
Slime.prototype.reproducirse = function(){
    let energíaNecesaria = MAX_ENERGIA/this.nivel;
    if(this.energía > energíaNecesaria){
        let cría;
        if(this.poder != undefined) {
            cría = new SlimeSanador(this.ataque, this.defensa, this.poder);
        } else {
            cría = new Slime(this.ataque, this.defensa);
        }
        cría.padre = this;
        this.energía -= energíaNecesaria;
        return cría;
    }
}

Slime.prototype.esDescendienteDe = function(otroSlime){
    
    let padreActual = this;
    
    while(padreActual.padre != undefined){
        if(padreActual.padre == otroSlime){
            return true;
        }
...
function testEjercicio6(res) {
    ...
    let slimeDefensivo2ReconoceASuPadre = slimeDefensivo2.esDescendienteDe(slimeDefensivo);
    ...
    res.write(`La cria del slime defensivo ${si_o_no(slimeDefensivo2ReconoceASuPadre)} reconoce a su padre.`, slimeDefensivo2ReconoceASuPadre);
    ...
    let slimeSanador2ReconoceASuPadre = slimeSanador2.esDescendienteDe(slimeSanador);
    ...
    res.write(`La cria del slime sanador ${si_o_no(slimeSanador2ReconoceASuPadre)} reconoce a su padre.`, slimeSanador2ReconoceASuPadre);
    ...
    let slimeSanador3ReconoceASuAbuelo = slimeSanador2.esDescendienteDe(slimeSanador);
    ...
    res.write(`La cria de la cria del slime sanador ${si_o_no(slimeSanador3ReconoceASuAbuelo)} reconoce a su abuelo.`, slimeSanador3ReconoceASuAbuelo);
